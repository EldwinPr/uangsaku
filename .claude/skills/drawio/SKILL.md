---
name: drawio
description: Always use when user asks to create, generate, draw, or design a diagram, flowchart, architecture diagram, ER diagram, sequence diagram, class diagram, network diagram, mockup, wireframe, or UI sketch, or mentions draw.io, drawio, drawoi, .drawio files, or diagram export to PNG/SVG/PDF. In this project, use for BPMN as-is/to-be diagrams, ERD, class diagrams, state diagrams, and the component diagram tracked under docs/diagrams/.
---
...
**Project convention:** diagrams for this repo live in `docs/diagrams/` and follow the naming already established there (`bpmn-as-is.drawio`, `bpmn-to-be.drawio`, `erd.drawio`, `class-conceptual.drawio`, `class-implementation.drawio`, `state-{entity}.drawio`, `component.drawio`). When asked to (re)generate one of these, write/update the file at that exact path rather than a new filename. For BPMN notation choices, defer to `context/document-writer-only/bpmn-conventions.md` when it exists.
...
| **XML** | Custom styling, precise/hand positioning, specific shape libraries (AWS, Azure, network, UML detail, BPMN…), or when the desktop CLI is not installed | No (optional ELK `--layout` needs the CLI) |
...
- **Use XML** for precise control (this applies to BPMN diagrams in this project — Mermaid has no native BPMN diagram type, so BPMN pools/lanes/gateways/events must be authored as draw.io XML using the BPMN shape library), or as the universal fallback: XML needs no CLI at all, so it's the only option when the desktop app isn't installed (output a `.drawio` file or a `url`).
...
2. **Deliver** (identical for both sources):
   ...
   - **png / svg / pdf** → export from the `.drawio` with embedded XML, then delete the source `.drawio` (unless the `.drawio` is a tracked project file under `docs/diagrams/`, in which case keep it — only delete throwaway intermediates):
     ```bash
...
Note: BPMN pool/lane diagrams generally look wrong under auto-layout (lanes must stay horizontal bands, sequence flow must read left-to-right within a lane) — hand-position BPMN XML or use `horizontalFlow` cautiously and check the result rather than trusting ELK blindly.
...
Match the language of the diagram labels to the user's language (this project's proposal/thesis and elicitation are in Indonesian — default BPMN/ERD labels to Indonesian unless told otherwise).

## Choosing the output format

Check the user's request for a format preference. Examples:

- `create a flowchart` → Mermaid → `flowchart.drawio`
- `png flowchart for login` → Mermaid → `login-flow.drawio.png`
- `svg: ER diagram` → Mermaid → `er-diagram.drawio.svg`
- `pdf AWS architecture overview` → XML (needs AWS shapes) → `architecture-overview.drawio.pdf`
- `url flowchart for user login` → opens browser at `app.diagrams.net` with the diagram, keeps `login-flow.drawio` locally
...
This project runs on Windows (PowerShell primary). Try:

```
"C:\Program Files\draw.io\draw.io.exe"
```

Use `which drawio` (or `where draw.io` on Windows) to check if it's on PATH before falling back to the platform-specific path. If not found, tell the user and fall back to XML authoring + `url` output — do not block on the CLI being installed.
...
- For diagrams already tracked in `docs/diagrams/` (see project convention above), reuse the existing filename — don't invent a new one.
- For new, untracked diagrams: use a descriptive filename based on the diagram content (e.g., `login-flow`, `database-schema`), lowercase with hyphens for multi-word names.
...
- After a successful export of a throwaway/intermediate `.drawio`, delete it — the exported file contains the full diagram. Do not delete a `.drawio` that is a tracked project artifact under `docs/diagrams/`.
...
Adapted from the official draw.io Claude Code plugin: https://github.com/jgraph/drawio-mcp (plugins/claude-code/skills/drawio/SKILL.md). If this project ever needs the full MCP tool server (open_drawio_xml / search_shapes across 10,000+ shapes), install it separately via `/plugin marketplace add jgraph/drawio-mcp` — this file is a standalone copy, not a live sync.
